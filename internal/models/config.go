...
type YAMLConfig struct {
	Checks []struct {
		...
		HTTP            struct {
			...
			Method         string `yaml:"method" env-description:"HTTP method" env-default:"GET"`
			ExpectedBody   string `yaml:"expected_body" env-description:"Operates as a basic 'body should contain <string>'" env-default:"nil"`
			ExpectedStatus int    `yaml:"espected_status" env-description:"Expected response status code" env-default:"200"`
		} `yaml:"http,omitempty"`
	} `yaml:"checks"`
	Containerd struct {
